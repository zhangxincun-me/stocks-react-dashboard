...
import baostock as bs  # 新增 BaoStock
import duckdb
import feedparser
import numpy as np
import pandas as pd
...
from llm_predictor import LLMStockPredictor
...
MARKET_HOURS = {
    ...
    'CN': {'timezone': 'Asia/Shanghai', 'open': '09:30', 'close': '15:00', 'days': [0, 1, 2, 3, 4]},  # 新增 A 股
...
def fetch_stock_data(ticker: str, period: str = "1y"):
    """Fetch stock data using BaoStock with DuckDB caching"""
    try:
        ...
        if period == '1mo':
            ...
            start_date = end_date - timedelta(days=3650)  # 默认最长 10 年
        ...
        # 3. 将 BaoStock 数据转换为 yfinance 的标准 DataFrame 格式
        df = pd.DataFrame(data_list, columns=rs.fields)
        # 转换数值类型
        for col in ['open', 'high', 'low', 'close', 'volume']:
            df[col] = pd.to_numeric(df[col], errors='coerce')

        # 统一列名和索引以适配你原有的所有预测模型
        ...
        info = {
            ...
            'currency': 'CNY',
            'exchange': 'SSE' if bs_ticker.startswith('sh') else 'SZSE',
            'symbol': ticker.upper(),
            'marketCap': None,
            'sector': 'Industrials/Tech',  # 面向看板的默认占位符
        }
        ...
        # 缓存入 DuckDB
        cache_data(ticker, period, (df, info), 'stock',
                   'open' if market_open else 'closed', exchange)

        return df, info, datetime.now(), market_status, exchange
...
async def search_stock(request: StockSearchRequest):
    """Search for stock ticker (Optimized for A-Shares)"""
    try:
        ...
        # 1. 如果输入的是纯数字代码 (如 600406) 或带前缀 (如 sh.600406)
        if query.replace('.', '').replace('sh', '').replace('sz', '').isdigit():
            formatted_ticker = format_baostock_ticker(query)
            clean_code = formatted_ticker.split('.')[-1]
            exchange_name = "Shanghai" if formatted_ticker.startswith('sh') else "Shenzhen"

            result = [{
                "ticker": formatted_ticker,
                "name": f"A-Share Stock ({clean_code})",
                "exchange": exchange_name,
                "type": "EQUITY"
            }]
            save_search_to_cache(query, formatted_ticker, result[0]['name'])
            return result

        # 2. 简单的中文名称映射 (可自行扩充你们关注的工业/电力股票)
        chinese_mappings = {
            '国电南瑞': 'sh.600406',
            '中国电建': 'sh.601669',
            '平安银行': 'sz.000001',
        ...
        for name, code in chinese_mappings.items():
            if query in name or name in query:
                result = [{
                    "ticker": code,
                    "name": name,
                    "exchange": "Shanghai" if code.startswith('sh') else "Shenzhen",
                    "type": "EQUITY"
                }]
                save_search_to_cache(query, code, name)
                return result

        # 3. 兜底返回 (假设用户输入的是正确的代码)
        return [{
            "ticker": query,
            "name": query.upper(),
            "exchange": "Unknown",
                    ...
                    if is_ticker_relevant or (is_analytical and is_market_context):
                        ...
                        if any(word in content_lower for word in ['upgrade', 'downgrade', 'rating', 'price target']):
                            ...
                            article_type = "Earnings Analysis"
                                 ...
                                 ['surge', 'plunge', 'rally', 'decline', 'jump', 'drop']):
                        ...
                        article_data = {
                            'title': entry.title,
                            'description': entry.get('summary', '')[:300] + "..." if len(
                                entry.get('summary', '')) > 300 else entry.get('summary', ''),
                            'source': source,
                            ...
                            'original_text': entry.get('summary', '')[:500] + "..." if len(
                                entry.get('summary', '')) > 500 else entry.get('summary', ''),
                            'article_type': article_type,
                            'relevance_score': relevance_score,
                            'sentiment': round(sentiment_score, 2)
                        }
...
async def llm_predict(request: LLMPredictionRequest):
    ...
    try:
        ...
        data, info, data_timestamp, market_status, exchange = fetch_stock_data(request.ticker,
                                                                               "1mo")  # Get last month for context
        if data is None or data.empty:
            return LLMPredictionResponse(
                success=False,
                message=f"Failed to fetch stock data for {request.ticker}"
