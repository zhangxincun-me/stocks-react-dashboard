...
下列截图展示系统主要界面与核心功能模块，用于辅助说明前端结构、交互入口、新闻展示和 AI 预测功能。

![图1：完整仪表板界面](docs_images/工程报告_3_image1.jpeg)

图1：系统主界面，展示股票搜索、行情指标、价格图表、预测设置和标签页布局。

![图2：左侧控制面板](docs_images/工程报告_2_image2.jpeg)

图2：控制参数区域，包含股票搜索、最近使用记录、预测方法、历史周期和预测天数设置。

![图3：新闻列表组件](docs_images/工程报告_1_image3.jpeg)

图3：新闻资讯模块，展示新闻标题、摘要、来源、发布时间、相关度评分和展开按钮。

![图4：AI预测模块](docs_images/工程报告_0_image4.jpeg)

图4：AI Prediction 模块，展示 LLM 股票预测、实时预测入口、回测分析入口和 DistilBERT 标识。
